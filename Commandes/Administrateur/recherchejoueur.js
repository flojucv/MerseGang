const { EmbedBuilder, ActionRowBuilder, ButtonBuilder, AttachmentBuilder, PermissionsBitField } = require('discord.js');
...
module.exports.run = async (client, message, args) => {
    ...
    if (!args[0]) return message.channel.send(`❌| Veuillez rentrer le nombre de joueur qui doivent vous rejoindre. \`${prefix}groupelive <nombre de joueur> <jeux>\``).then(message => { setTimeout(() => message.delete().catch(err => console.log(err)), 5000); });
    if (isNaN(args[0])) return message.channel.send(`❌| Vous devez rentrer un nombre. \`${prefix}groupelive <nombre de joueur> <jeux>\``).then(message => { setTimeout(() => message.delete().catch(err => console.log(err)), 5000); });
    if (parseInt(args[0]) <= 0) return message.channel.send(`❌| Vous ne pouvez pas lancez une recherche avec 0 joueur. \`${prefix}groupelive <nombre de joueur> <jeux>\``)
    if (!args[1]) return message.channel.send(`❌| Vous n'avez pas spécifiez de jeux. \`${prefix}groupelive <nombre de joueur> <jeux>\``)
    let place = 1;
    ...
    await message.channel.send({ content: '@here \nMersedi_ cherche des gens avec qui jouer !', embeds: [embedRechercheJoueur], components: [row], files: [attchment] }).then(async msg => {
