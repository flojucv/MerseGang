...
module.exports.run = async(client, message, args) => {
    ...
    .addFields(
        ...
        { name: "<:creele:1009367950194647120>| Créer le", value: `${moment(message.guild.members.cache.find(member => member.id === client.user.id).user.createdAt).format('DD/MM/YYYY')}`}
    )
    .setColor("#B4E0E0")

    message.channel.send({ embeds: [embedBotInfo]});
}
...
module.exports.runSlash = async(client, interaction) => {
    ...
    interaction.reply({ embeds: [embedBotInfo]});
